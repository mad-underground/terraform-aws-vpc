...
output "redshift_route_table_ids" {
  ...
  value       = length(aws_route_table.redshift[*].id) > 0 ? aws_route_table.redshift[*].id : (var.vpc.enable_public_redshift ? aws_route_table.public[*].id : aws_route_table.private[*].id)
}
...
output "nat_public_ips" {
  ...
  value       = var.vpc.reuse_nat_ips ? var.vpc.external_nat_ips : aws_eip.nat[*].public_ip
}
...
output "vpc_flow_log_destination_type" {
  ...
  value       = var.vpc.flow_log_destination_type
}
